...
void AndroidCameraInput::sendEventToPeer(uint8 format_type, int32 format_index,
        ...
        OsclAny* context)
{
    ...
    if (err != OsclErrNone)
    {
        LOGE("AndroidCameraInputListener::sendEventToPeer: Error - writeAsync failed. err=%x", err);
        return;
    }
}
...
PVMFStatus AndroidCameraInput::postWriteAsync(nsecs_t timestamp, const sp<IMemory>& frame)
{
    ...
    uint32 timeStamp = (uint32)(timestamp / 1000000L);
    ...
    if (iAudioFirstFrameTs == 0)
        iAudioFirstFrameTs = timeStamp;

    if (timeStamp < iAudioFirstFrameTs)
    {
        // Drop the frame
        mCamera->releaseRecordingFrame(frame);
        return PVMFSuccess;
